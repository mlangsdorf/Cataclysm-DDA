...
class cm_opts {
    private:
        ...
        std::vector<std::vector<mission_entry>> mission_key_vectors( NW_TAB - TAB_MAIN + 3 );
        std::map<std::string, std::string> col_missions;
        std::vector<std::shared_ptr<npc>> npc_list;

    public:
        cm_opts( std::vector<std::vector<mission_entry>> mission_key_vectors ): {};

};

// Replace most of this if statement and the function calls with a search through the
// companion_mission json objects, matching on conditionals. Pass a vector of options
// to display_and_choose_opts; each option has 1 of an assign function, group_assign
// function, or a recovery function. Execute the apppropriate function.
//
// Basically, replicate the dynamic_line/responses system of npctalk.cpp.
void talk_function::companion_mission( npc &p )
{
    ...
    if( id == "FACTION_CAMP" ) {
        ...
        faction_camp_missions( npc );
    } else if( id == "SCAVENGER" ) {
        title = _( "Junk Shop Missions" );
        junk_shop_scavenge( npc );
        ...
        if( !p.has_trait( trait_NPC_CONSTRUCTION_LEV_1 ) ) {
            crops_purchase( p );
        } else if ( p.has_trait( trait_NPC_CONSTRUCTION_LEV_1 ) ) {
        ...
        foreman_menial_labor( npc );
        if( p.has_trait( trait_NPC_MISSION_LEV_1 ) ) {
            foreman_carpentry( npc );
        }
    } else if( id == "REFUGEE MERCHANT" ) {
        title = _( "Free Merchant Missions" );
        commune_caravan( npc );
    }
    mission_entry cur_key = display_and_choose_opts( p, id, title );
    if( id == "FACTION_CAMP" ) {
        return camp_missions( p, id, title, cur_key );
    }
    return outpost_missions( p, id, title, cur_key );
}

void cm_opts::junk_shop_scavenge( npc &p )
{
    std::string entry;
    std::string entry_aux;
    col_missions["Assign Scavenging Patrol"] = json_lookup( "_scavenging_patrol" );
...
void cm_opts::junk_shop_raid( npc &p )
{
    col_missions["Assign Scavenging Raid"] = json_lookup( "_scavenging_raid" );
...
void cm_opts::foreman_menial_labor( npc &p )
{
    std::vector<std::shared_ptr<npc>> npc_list;
    col_missions["Assign Ally to Menial Labor"] = json_lookup( "_labor" );
...
void cm_opts::foreman_carpentry( npc &p )
{
    col_missions["Assign Ally to Carpentry Work"] = json_lookup( "_carpenter" );
...
// WTF are these even doing here? They should be dialogue options. They don't
// involve your npcs AT ALL.
void cm_opts::crops_purchase( npc &p )
{
    col_missions["Purchase East Field"] = json_lookup( "_commume_crop_purchase" );
    mission_key_push( mission_key_vectors, "Purchase East Field", _( "Purchase East Field" ) );
}

void cm_opts::crops_upgrade( npc &p )
{
    col_missions["Upgrade East Field I"] = json_lookup( "_commume_crop_upgrade_1" );
    mission_key_push( mission_key_vectors, "Upgrade East Field I", _( "Upgrade East Field I" ) );
}

void cm_opts::crops_work( npc &p )
{
    col_missions["Plant East Field"] = json_lookup( "_commume_crop_purchase" );
    mission_key_push( mission_key_vectors, "Plant East Field", _( "Plant East Field" ) );
    col_missions["Harvest East Field"] = json_lookup( "_commume_crop_purchase" );
    mission_key_push( mission_key_vectors, "Harvest East Field", _( "Harvest East Field" ) );
}

void cm_opts::commue_forage( npc &p )
{
    col_missions["Assign Ally to Forage for Food"] = json_lookup( "_forage" );
...
void cm_opts::commune_caravan( npc &p )
{
    col_missions["Caravan Commune-Refugee Center"] = json_lookup( "_commune_refugee_caravan" );
...
mission_entry cm_opts::display_and_choose_opts( npc &p, const std::string &id, const std::string &title ) {
    if( col_missions.empty() ) {
        popup( _( "There are no missions at this colony.  Press Spacebar..." ) );
        return false;
    }

    int TITLE_TAB_HEIGHT = ( id == "FACTION_CAMP" ) ? 1 : 0;
...
bool talk_function::outpost_missions( npc &p, const std::string &id, const std::string &title, mission_entry &cur_key )
{
