...
point vehicle::rotate_mount( int old_dir, int new_dir, const point &pivot,
                             const point &p ) const
{
    tripoint q;
...
std::set<int> vehicle::precalc_mounts( int idir, int dir, const point &pivot )
{
    if( idir < 0 || idir > 1 ) {
        idir = 0;
    }
...
{
    assert( static_cast<int>( part_index() ) < vehicle().num_parts() );
